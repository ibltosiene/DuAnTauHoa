...
const BookingSearch = ({ onSuccess, onError, setIsLoading }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!don) { onError('Không tìm thấy thông tin đặt chỗ. Vui lòng kiểm tra lại.'); return }
      onSuccess(normalizeApiBooking(don))
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit}
      ...
      <div className="mb-4 text-center">
        <h2 className="text-xl font-bold text-[#8C1D19]">TRA CỨU ĐẶT CHỖ</h2>
        <p className="text-[#8C1D19]/70 text-xs">Nhập thông tin để tra cứu đơn đặt vé</p>
      </div>
      ...
      <div className="relative mb-3">
        ...
        <input type="text" name="bookingCode" value={form.bookingCode}
          onChange={handleChange} placeholder="Mã đặt chỗ (VD: ABC123)"
      ...
      <div className="flex gap-3 mb-4">
        <button type="submit" disabled={loading}
          className="flex-1 flex h-11 items-center justify-center gap-2 rounded-md bg-[#FFD15A] text-sm font-bold text-[#8C1D19] hover:bg-[#ffe082] disabled:opacity-60 transition-colors">
          <FaMagnifyingGlass className="h-4 w-4" />
          {loading ? 'Đang tra cứu...' : 'Tra cứu'}
        </button>
        <button type="button" onClick={handleReset}
          className="flex h-11 items-center justify-center gap-2 rounded-md bg-[#8C1D19] px-4 text-sm font-bold text-white hover:bg-[#7a1916] transition-colors">
      ...
      <div className="rounded-md bg-amber-50/80 border border-amber-300 p-3 text-xs text-amber-800">
        ...
        <ul className="space-y-0.5">
          <li>• Nhập đúng <strong>mã đặt chỗ</strong>, số điện thoại và email đã dùng khi đặt vé</li>
          <li>• Mã đặt chỗ gồm <strong>6 ký tự</strong> viết hoa</li>
          <li>• Liên hệ hotline nếu cần hỗ trợ tra cứu</li>
        </ul>
      </div>
