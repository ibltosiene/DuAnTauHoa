...
const { DonDatVe, Ve, TamGiuGhe, sequelize } = require('../models')
...
router.post('/:idDon/mark-paid', async (req, res, next) => {
  try {
    ...
    ok(res, {
      ...
      veList: veList.map(v => ({
        idVe: v.id_ve, idChuyen: v.id_chuyen, soToa: v.so_toa_thu_tu,
        soGhe: v.so_ghe_trong_toa, giaVe: parseFloat(v.gia_ve), loaiHanhKhach: v.loai_hanh_khach,
      })),
    })
  } catch (err) { next(err) }
})
